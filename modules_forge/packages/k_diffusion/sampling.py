...
def sample_dpmpp_sde_cfg_pp(model, x, sigmas, extra_args=None, callback=None, disable=None, noise_sampler=None):
    ...
    # Use half-logSNR so const / flow predictors (Anima, Flux, Wan) get the right
    # log-time domain, matching the non-CFG++ sample_dpmpp_sde.
    model_sampling = model.inner_model.predictor
    lambda_fn = partial(sigma_to_half_log_snr, model_sampling=model_sampling)
    sigma_fn = partial(half_log_snr_to_sigma, model_sampling=model_sampling)
    ...
    for i in trange(len(sigmas) - 1, disable=disable):
        ...
        if sigmas[i + 1] == 0:
            ...
            t, t_next = lambda_fn(sigmas[i]), lambda_fn(sigmas[i + 1])
            h = t_next - t
            s = t + h * r
            fac = 1 / (2 * r)
...
def sample_dpmpp_2m_cfg_pp(model, x, sigmas, extra_args=None, callback=None, disable=None):
    ...
    s_in = x.new_ones([x.shape[0]])
    # Use half-logSNR so const / flow predictors (Anima, Flux, Wan) get the right
    # log-time domain, matching the non-CFG++ sample_dpmpp_2m.
    model_sampling = model.inner_model.predictor
...
def sample_dpmpp_2m_sde_cfg_pp(model, x, sigmas, extra_args=None, callback=None, disable=None, eta=1.0, s_noise=1.0, noise_sampler=None, solver_type="midpoint"):
    ...
    # Use half-logSNR so const / flow predictors (Anima, Flux, Wan) get the right
    # log-time domain, matching the non-CFG++ sample_dpmpp_2m_sde.
    model_sampling = model.inner_model.predictor
    lambda_fn = partial(sigma_to_half_log_snr, model_sampling=model_sampling)
    ...
    for i in trange(len(sigmas) - 1, disable=disable):
        ...
        if sigmas[i + 1] == 0:
            ...
            t, s = lambda_fn(sigmas[i]), lambda_fn(sigmas[i + 1])
            h = s - t
            eta_h = eta * h
            h_eta = h * (eta + 1)

            # alpha_t = sigmas[i+1] * exp(lambda_t). For const (Anima, Flux, Wan)
            # lambda = -logit(sigma), so exp(lambda) = (1-sigma)/sigma, giving
            # alpha_t = 1 - sigmas[i+1]. For non-const (EDM), lambda = -log(sigma),
            # so exp(lambda) = 1/sigma, giving alpha_t = 1.
            alpha_t = sigmas[i + 1] * s.exp()

            x = sigmas[i + 1] / sigmas[i] * (-eta_h).exp() * (x + (denoised - temp[0])) + alpha_t * (-h_eta).expm1().neg() * denoised
...
def sample_dpmpp_3m_sde_cfg_pp(model, x, sigmas, extra_args=None, callback=None, disable=None, eta=1.0, s_noise=1.0, noise_sampler=None):
    ...
    sigmas = offset_first_sigma_for_snr(sigmas, model_sampling)
    # Use half-logSNR so const / flow predictors (Anima, Flux, Wan) get the right
    # log-time domain, matching the non-CFG++ sample_dpmpp_3m_sde.
    lambda_fn = partial(sigma_to_half_log_snr, model_sampling=model_sampling)
    ...
    for i in trange(len(sigmas) - 1, disable=disable):
        ...
        if sigmas[i + 1] == 0:
            ...
            x = sigmas[i + 1] / sigmas[i] * (-h * eta).exp() * (x + (denoised - temp[0])) + alpha_t * (-h_eta).expm1().neg() * denoised
...
def _dpm_solver_step(x, t, s, denoised, denoised_1, denoised_2, h_1, h_2, eta, noise_sampler, s_noise,
    ...
    # alpha_t = 1 - sigmas[i+1]. For non-const (EDM), lambda = -log(sigma),
    # so exp(lambda) = 1/sigma, giving alpha_t = 1.
    alpha_t = sigma_s * s.exp()

    # Standard DPM-Solver++ First Order (CFG++ form: shift x into uncond's frame)
    x = sigma_s / sigma_t * (-h * eta).exp() * (x + (denoised - uncond_denoised)) + alpha_t * (-h_eta).expm1().neg() * denoised
...
def sample_dpmpp_3m_sde_cfgpp_ctrlz(model, x, sigmas, extra_args=None, callback=None, disable=None, eta=1., s_noise=1., noise_sampler=None, gamma_scale=0.5):
    ...
    for i in trange(len(sigmas) - 1, disable=disable):
        ...
        if sigma_s == 0:
            # Final step: use proper Euler step instead of just returning denoised.
            # Matches sample_dpmpp_3m_sde_cfg_pp's final-step behavior.
            d = to_d(x, sigma_t, uncond_denoised)
            dt = 0 - sigma_t
            x = x + d * dt
            break

        # Convert sigma to half-logSNR so the solver step uses the same t-domain
        # as the non-CFG++ sample_dpmpp_3m_sde.
        t, s = lambda_fn(sigma_t), lambda_fn(sigma_s)
